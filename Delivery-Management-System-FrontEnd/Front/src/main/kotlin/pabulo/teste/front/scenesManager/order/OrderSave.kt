...
import pabulo.teste.front.controllers.order.OrderSaveController
...
class OrderSave(private val saveOrder: Stage) {

    private lateinit var stageInit: Stage
    private lateinit var scenesManager: ScenesManager

    fun saveOrderInit(stageInit: Stage) {

        this.stageInit = stageInit
        scenesManager = ScenesManager(saveOrder)


        val fxmlLoader = FXMLLoader(Main::class.java.getResource("/pabulo/teste/front/order/order-save.fxml"))
        val orderSaveScene = Scene(fxmlLoader.load(), 1535.0, 851.0)
        val cssMenu = Main::class.java.getResource("/pabulo/teste/front/order/order-style.css")?.toExternalForm()
        orderSaveScene.stylesheets.add(cssMenu)

        val controller = fxmlLoader.getController<OrderSaveController>()
        controller.setOrderSaveApp(this)

        saveOrder.scene = orderSaveScene

        saveOrder.isResizable = false
